...
const EditHousePlan = (props) => {
    ...
    const onSubmit = async(event) => {
        ...
        console.log("in edit" + props._id);

        const formData = new FormData(event.target);
        console.log(...formData);

        const response = await fetch(`http://localhost:3001/api/house/${props._id}`,{
            method:"PUT",
            body:formData
        });

        if(response.status === 200) {
            setResult("Hoise Plan edited succesfully");
            event.target.reset();
            props.closeEditDialog();
        } else {
            setResult("Error editing your house plan");
        }
    };
